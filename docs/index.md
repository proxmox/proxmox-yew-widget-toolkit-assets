<link rel="stylesheet" type="text/css" href="css/desktop-yew-style.css" />
<link rel="stylesheet" type="text/css" href="css/font-awesome.css" />
<link rel="stylesheet" type="text/css" href="css/doc-style.css" />

# Proxmox Widget Toolkit CSS Styles

Welcome to the documentation for the Proxmox Widget CSS Styles. Below you will find detailed guides and examples for the available layout and styling classes.

## Widgets

- [**Buttons**](button.md) - Standard, Outlined, Elevated, and Segmented buttons.
- [**Cards**](cards.md) - Flexible containers with various layout and elevation options.
- [**Form Inputs**](inputs.md) - Text fields, number inputs, and text areas.
- [**Panels & Containers**](panels.md) - General purpose panels and grid layouts.
- [**Selection Controls**](selection.md) - Checkboxes, Radio Buttons, Toggle Switches, and Comboboxes.

## Resources

- [**CSS Class Cheatsheet**](cheatsheet.html) - A dense, printable reference for all CSS classes.
